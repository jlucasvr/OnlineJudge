...
export default function App() {
  ...
  // Definimos que o ref guardará uma instância de IStandaloneCodeEditor
  const editorRef = useRef<editor.IStandaloneCodeEditor | null>(null);

  // A tipagem OnMount já define os parâmetros (editor, monaco)
  ...
  return (
    <div className='grid grid-rows-[auto_1fr_auto] grid-cols-[auto_1fr] min-h-screen bg-zinc-900'>

      <header className='h-20 col-span-2 flex items-center space-x-4 p-4'>
        <button
          className='p-2 bg-zinc-50 hover:bg-orange-600 active:scale-95 transition cursor-pointer rounded-xl text-2xl text-zinc-950 font-bold'
        ...
        <span className='text-3xl text-orange-600 font-black'>Pumpk<span className='text-purple-600'>in</span> Code</span>
      </header>
      ...
      <main className='p-10 flex space-x-10 overflow-y-auto rounded-l-[48px] bg-zinc-950'>
        ...
        <Card className='w-2/5 text-white'>
          <div className='p-4 space-y-4 grid grid-rows-[auto_1fr_auto]'>
              ...
              {langs.map((lang) => (
                <label key={lang.value} className="relative">
                  ...
                  <div className="px-4 py-2 font-bold bg-zinc-800 border border-zinc-700 rounded-xl cursor-pointer
                      peer-checked:border-orange-600 peer-checked:text-orange-500 transition-all">
                    {lang.label}
                  </div>
                </label>
              ))}
            ...
            <button onClick={handleSubmit} className='py-2 px-4 bg-zinc-50 hover:bg-orange-600 active:scale-95 transition cursor-pointer rounded-xl text-zinc-950 font-bold justify-self-end'>Submeter</button>
          </div>
        </Card>
      </main >
